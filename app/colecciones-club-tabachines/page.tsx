...
export default function ColeccionesClubTabachines() {
  const { content, language } = useLanguage();
  const { colecciones } = content;

  const pageTitle = language === "es" 
    ? "COLECCIONES: VALERIA MEZA & ARREBOL WEDDINGS"
    : "COLLECTIONS: VALERIA MEZA & ARREBOL WEDDINGS";
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <FadeIn delay={200}>
            <p className="mb-8 text-lg text-[var(--foreground)]/80 leading-relaxed text-center max-w-3xl mx-auto">
              Paquetes especiales: 9 de enero 2027, Club de Golf Tabachines
            </p>
            ...
            {colecciones.collections.map((collection, index) => (
              <FadeIn key={index} delay={index * 100}>
                <div className="bg-white border border-[var(--border-subtle)] p-10 h-full flex flex-col">
                  ...
                  {/* Price at bottom - CON 50% DESCUENTO */}
                  <div className="pt-6 border-t border-[var(--border-subtle)] text-center">
                    {(() => {
                      const originalPrices = [59000, 72000, 95000, 110000];
                      const discountedPrices = [25000, 30500, 40500, 46500];
                      return (
                        <>
                          <p className="text-lg text-[var(--foreground)]/40 line-through mb-1">
                            ${originalPrices[index].toLocaleString('es-MX')} MXN
                          </p>
                          <p className="text-2xl font-light text-[var(--foreground)]">
                            ${discountedPrices[index].toLocaleString('es-MX')} <span className="text-sm text-[var(--foreground)]/50 uppercase tracking-wider">MXN</span>
                          </p>
                        </>
                      );
                    })()}
                      ...
                      return (
                        <td key={index} className="p-4 text-center">
                          <div className="text-sm text-[var(--foreground)]/40 line-through mb-1">
                            ${originalPrices[index].toLocaleString('es-MX')}
                          </div>
                          <div className="text-2xl font-light text-[var(--foreground)]">
                            ${discountedPrices[index].toLocaleString('es-MX')}
                          </div>
