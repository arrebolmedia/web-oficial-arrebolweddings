...
export default function ColeccionesAlejandraSalgado() {
  ...
  // Precios con incremento de $10,000
  const collections = colecciones.collections.map(collection => {
    const priceStr = collection.price.split(' ')[0].replace(/[^0-9]/g, '');
    const priceNum = parseInt(priceStr);
    const newPrice = priceNum + 30000; // 20000 original + 10000 adicional
    return {
      ...collection,
  ...
  return (
    <>
      ...
      <FadeIn>
        <SectionHeader
          title="PARA ALEJANDRA SALGADO"
          subtitle="Los Hilos, San Miguel de Allende"
                ...
                <div className="bg-white border border-[var(--border-subtle)] p-10 h-full flex flex-col">
                  ...
                  <div className="pt-6 border-t border-[var(--border-subtle)] text-center">
                    <p className="text-2xl font-light text-[var(--foreground)]">
                      ${collection.adjustedPrice.toLocaleString('es-MX')} <span className="text-sm text-[var(--foreground)]/50 uppercase tracking-wider">{collection.price.split(' ')[1]}</span>
                    </p>
                  </div>
                </div>
              </FadeIn>
                    ...
                    {collections.map((collection, index) => (
                      <td key={index} className="p-4 text-center">
                        <div className="text-2xl font-light text-[var(--foreground)]">
                          ${collection.adjustedPrice.toLocaleString('es-MX')}
                        </div>
